import { Events, CommandInteraction, ButtonInteraction, ModalSubmitInteraction, AutocompleteInteraction, type ApplicationCommandOptionChoiceData } from 'discord.js'
...
import { monsterSearchResultLimit, monsterSearchThreshold, playerSearchResultLimit, playerSearchThreshold } from '../../misc/constants/index.ts'
...
export default new DiscordEvent({
  ...
  async run (client, interaction) {
    ...
    if (interaction instanceof CommandInteraction) {
            ...
            new CustomEmbed()
              ...
              .setDescription(
                client.translate('INTERACTION_UNKNOWN_COMMAND_DESCRIPTION', args.language, {
                  date: args.user.vip_data?.expiration_date !== undefined ? `<t:${Math.floor(new Date(args.user.vip_data.expiration_date).getTime() / 1000)}>` : '` `'
                })
              )
      ...
      const cooldown = client.handleInteractionCooldown(`${Events.InteractionCreate}-${interaction.commandName}-${interaction.user.id}`, command.options.cooldown * 1000)
        ...
        await interaction.reply({
          embeds: [
            new CustomEmbed()
              ...
              .setDescription(client.translate('INTERACTION_COOLDOWN_COMMAND_DESCRIPTION', args.language, { time: Math.round(cooldown / 1000) }))
              .setAuthor({ name: 'Kakele Biridim', iconURL: client.icons.ElderVampireBrooch })
              .setColor(client.colors.DarkRed)
          ],
          ephemeral: true
      ...
      if (args.guild?.blacklisted !== undefined) {
        const ban = args.guild.blacklisted.find((x) => (x.commands?.includes(interaction.commandName) || x.commands?.includes('all')) && new Date(x.expiration_date) > new Date())
      ...
      switch (interaction.customId) {
        ...
        case 'MARKETPLACE_VIEW_DM_USER_REPORT':
        case 'MARKETPLACE_VIEW_SELECT_ITEM_REPORT':
          await marketplace.showReportItemModal(client, interaction, args.language)
          break

        case 'MARKETPLACE_VIEW_SELECT_ITEM_CHAT':
          await marketplace.showMessageToOwnerModal(client, interaction, args.language)
          break

        case 'MARKETPLACE_VIEW_DM_USER_ANSWER':
          await marketplace.showMessageAnswerModal(client, interaction, args.language)
          break
      ...
      switch (interaction.customId) {
        ...
        case 'MARKETPLACE_VIEW_REPORT_MODAL':
          await marketplace.handleReportItemSubmit(client, interaction, args.language)
          break

        case 'MARKETPLACE_VIEW_MESSAGE_MODAL':
          await marketplace.handleMessageSubmit(client, interaction, args.language)
          break

        case 'MARKETPLACE_VIEW_ANSWER_MODAL':
          await marketplace.handleMessageSubmit(client, interaction, args.language)
          break
