import { ChannelSelectMenuBuilder, ChannelType, RoleSelectMenuBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder } from 'discord.js'
import type Biridim from '../../structures/client.ts'
import { type IOfferItemDocument } from '../../database/schemas/marketplace-item.ts'
import { type KakeleMonster, type CharacterData, type SupportedLanguagesString, type KakeleItem, type SupportedLanguages } from '../../structures/misc.ts'
  ...
  data.forEach((option, index) => {
    options.push(
      new StringSelectMenuOptionBuilder()
        ...
        .setDescription(`${getLevel(Number(option.item.experience))} ${capitalizeFirstLetter(data[index].item.vocation.toLowerCase())} | ${capitalizeFirstLetter(option.item.server ?? '')}`)
    )
  })
...
export function MONSTER_RESULT_SELECTION_MENU (data: Array<{ item: KakeleMonster }>, language: SupportedLanguagesString): StringSelectMenuBuilder {
  const options: StringSelectMenuOptionBuilder[] = []
  data.forEach((option, index) => {
    options.push(
      new StringSelectMenuOptionBuilder()
...
export function MARKETPLACE_ADD_RESULT_SELECTION_MENU (client: Biridim, items: any[], language: string): StringSelectMenuBuilder {
  ...
  items.forEach(item => {
    const emoji = client.kakeleEmojis.find(x => x.name === item.name.english
      .replace(/[^A-Z0-9]/ig, '')
    )
