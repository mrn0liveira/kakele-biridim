...
import { SupportedLanguages, type InteractionArgs } from '../../structures/misc.ts';
...
export default new InteractionCommand({
  data: new SlashCommandBuilder()
    .setName('language')
    .setDescription('Sets the language to be used by the bot in guides and user commands.')
    .setDescriptionLocalizations({
      'es-ES': 'Establece el idioma que utilizará el bot en guías y comandos de usuario.',
      'pt-BR': 'Define o idioma a ser usado pelo bot em guias e comandos de usuário.',
    ...
    .addStringOption((option) =>
      option
        ...
        .setRequired(true)
        .addChoices({ name: 'English', value: 'en' }, { name: 'Português', value: 'pt' }, { name: 'Español', value: 'es' }, { name: 'Polski', value: 'pl' }),
    )
    .addStringOption((option) =>
      option.setName('target').setDescription('Target type to be configured').setRequired(true).addChoices({ name: 'Guild', value: 'guild' }, { name: 'User', value: 'user' }),
    ),
  options: {
  ...
  async run(interaction: ChatInputCommandInteraction<'cached'>, args: InteractionArgs) {
    const language = SupportedLanguages[interaction.options.getString('language')?.toUpperCase() ?? ''] ?? SupportedLanguages.EN;
    ...
    if (target === 'guild' && args.guild === undefined) {
      return await interaction.editReply({
        embeds: [
          new CustomEmbed()
            .setTitle(client.translate('COMMAND_LANGUAGE_GUILD_ONLY_TITLE', language))
            .setDescription(client.translate('INTERACTION_TIP_REPORT', language))
    ...
    async function setGuildLanguage(): Promise<void> {
      if (!interaction.member.permissions.has(PermissionFlagsBits.ManageChannels)) {
        await interaction.editReply({
          embeds: [
            new CustomEmbed()
              .setTitle(client.translate('COMMAND_LANGUAGE_MISSING_PERMISSION_TITLE', language))
              .setDescription(client.translate('INTERACTION_TIP_REPORT', language))
              .setAuthor({ name: 'Kakele Biridim', iconURL: client.icons.ElderVampireBrooch })
              .setColor(client.colors.DarkRed),
          ],
      ...
      logger.audit('setGuildLanguage', 'userId' + interaction.user.id, 'guildId' + interaction.guildId, language);
      ...
      await interaction.editReply({
        embeds: [
          new CustomEmbed()
            .setTitle(client.translate('COMMAND_LANGUAGE_GUILD_CHANGED_TITLE', language))
            .setDescription(client.translate('INTERACTION_TIP_REPORT', language))
            .setAuthor({ name: 'Kakele Biridim', iconURL: client.icons.ElderVampireBrooch })
    ...
    async function setUserLanguage(): Promise<void> {
      ...
      await interaction.editReply({
        embeds: [
          new CustomEmbed()
            .setTitle(client.translate('COMMAND_LANGUAGE_USER_CHANGED_TITLE', language))
            .setDescription(client.translate('INTERACTION_TIP_REPORT', language))
            .setAuthor({ name: 'Kakele Biridim', iconURL: client.icons.ElderVampireBrooch })
            .setColor(client.colors.LimeGreen),
        ],
