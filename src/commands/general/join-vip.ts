import { type ChatInputCommandInteraction, PermissionFlagsBits, SlashCommandBuilder, ActionRowBuilder, type ButtonBuilder } from 'discord.js'
  ...
  async run (interaction: ChatInputCommandInteraction<'cached'>, args: InteractionArgs) {
    ...
    if (attachment == null) {
      return await interaction.editReply({
        embeds: [
          new CustomEmbed()
            .setTitle(client.translate('COMMAND_JOIN_VIP_UNKNOWN_ATTACHMENT_TITLE', args.language))
            .setDescription(client.translate('COMMAND_JOIN_VIP_UNKNOWN_ATTACHMENT_DESCRIPTION', args.language, { date: args.user.vip_data?.expiration_date !== undefined ? `<t:${Math.floor(new Date(args.user.vip_data.expiration_date).getTime() / 1000)}>` : '` `' }))
      ...
      try {
        await client.sendMessageToGuildChannel(process.env.DISCORD_JOIN_VIP_CHANNEL as string,
          {
            embeds: [
              new CustomEmbed()
                .setTitle(client.translate('COMMAND_JOIN_VIP_CONFIRM_PAYMENT_TITLE', args.language))
        ...
        await interaction.editReply({
          embeds: [
            new CustomEmbed()
              .setTitle(client.translate('COMMAND_JOIN_VIP_SUCCESSFULLY_REQUESTED_TITLE', args.language))
              .setDescription(client.translate('COMMAND_JOIN_VIP_SUCCESSFULLY_REQUESTED_DESCRIPTION', args.language))
              .setFooter({ text: client.translate('COMMAND_JOIN_VIP_SUCCESSFULLY_REQUESTED_FOOTER', args.language) })
              .setAuthor({ name: 'Kakele Biridim', iconURL: client.icons.Cherry })
              .setColor(client.colors.LimeGreen)
          ]
        })

        const user = await User.findOne({ id: interaction.user.id }) ?? args.user
        ...
        return await interaction.editReply({
          embeds: [
            new CustomEmbed()
              .setTitle(client.translate('COMMAND_JOIN_VIP_ERROR_TITLE', args.language))
              .setDescription(client.translate('COMMAND_JOIN_VIP_ERROR_DESCRIPTION', args.language))
              .setAuthor({ name: 'Kakele Biridim', iconURL: client.icons.Cherry })
