...
import { type SupportedLanguages, type InteractionArgs, type KakeleMonster } from '../../structures/misc.ts'
...
async function createMonsterInfoImage (client: Biridim, monster: KakeleMonster, languageCode: SupportedLanguages): Promise<Canvas> {
  const language = getLanguageName(languageCode.toString())
  ...
  const monsterImageFilePath = `./src/assets/sprites/monsters/${monster.name.english}.png`
  ...
  const rawLoot = monster.loot?.split(', ') ?? []
  ...
  rawLoot.forEach((drop) => {
    const item = client.kakeleItems.find((x) => x.name.english === drop)
  ...
  for (const item of loot) {
    ...
    if (fs.existsSync(`./src/assets/sprites/items/${item.name}.png`)) {
      const avatar = await loadImage(`./src/assets/sprites/items/${item.name}.png`)
  ...
  if (monster.boss === true) {
    ...
    ctx.fillText('Boss', 620, monster.name[language].length <= 16 ? 138 : 138)
  }

  ctx.font = '60pt Teko'

  ctx.fillStyle = '#b8b8b8'
  ctx.fillText(monster.name[language], 620, 200)

  ctx.font = '25pt HelveticaBold'
  ctx.fillStyle = '#ff5f4f'
  ctx.fillText(monster.health, 670, 237)

  ctx.fillStyle = '#ffe536'
  ctx.fillText(monster.experience, 670, 283)

  ctx.fillStyle = '#de9021'
  ctx.fillText(monster.gold, 670, 331)
...
export default new InteractionCommand({
  ...
  async run (interaction: ChatInputCommandInteraction<'cached'>, args: InteractionArgs) {
    ...
    if (result.length > 1) {
      ...
      const actionRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(MONSTER_RESULT_SELECTION_MENU(result, language))
      ...
      await interaction.editReply({
        files: [
          {
            name: `${interaction.commandName}-${result[0].item.name.english}.png`,
            attachment: image.toBuffer()
          }
        ],
        embeds: [],
