...
import { CustomEmbed, capitalizeFirstLetter, generateDarkRandomColorHex } from '../../misc/util/index.ts';
...
async function createEventCountdownImage(data: IEvent[], language: SupportedLanguages): Promise<Canvas> {
  ...
  data.forEach((event, index) => {
    ...
    ctx.drawImage(overlay, 0, 0 + index * 40 + header.naturalHeight, overlay.naturalWidth, overlay.naturalHeight);
...
export default new InteractionCommand({
  ...
  async run(interaction: ChatInputCommandInteraction<'cached'>, args: InteractionArgs) {
    ...
    const events: IEvent[] = (await getKakeleEvents({})).filter((x) => Number(x.activation_enabled_unix_seconds) > new Date().getTime() / 1000 && x.server === server) ?? [];

    if (events.length === 0) {
      await interaction.editReply({
        embeds: [
          new CustomEmbed()
            .setTitle(client.translate('EVENT_COUNTDOWN_NO_EVENTS_TITLE', args.language))
    ...
    const image = await createEventCountdownImage(
      events.sort((a, b) => parseFloat(a.activation_enabled_unix_seconds) - parseFloat(b.activation_enabled_unix_seconds)),
      args.language,
    );
