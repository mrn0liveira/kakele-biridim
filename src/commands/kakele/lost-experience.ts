...
export async function createDailyLostRankingImage (client: Biridim, data: CharacterData[], server: string, language: SupportedLanguages): Promise<Canvas> {
  ...
  const backgroundImagePath = server === 'global' ? './src/assets/lost-experience/1.png' : './src/assets/lost-experience/2.png'
  ...
  ctx.fillText(client.i18n.__({ phrase: 'DAILY_LOST_EXPERIENCE_TITLE', locale: language }), 30, 65)

  ctx.fillStyle = '#ffecd6'
  ctx.fillText(obj.server, 30, 105)

  const formatted = data
    .sort((a, b) => parseFloat(String(a.progress)) - parseFloat(String(b.progress)))
