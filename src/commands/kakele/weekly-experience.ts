...
export async function createWeeklyRankingImage (client: Biridim, data: CharacterData[], server: string, language: SupportedLanguages): Promise<Canvas> {
  ...
  ctx.fillText(client.i18n.__({ phrase: 'WEEKLY_EXPERIENCE_TITLE', locale: language }), 30, 65)
  ...
  for (let i = 0; i < formatted.length; i += 1) {
    ...
    fillMixedText(
      ...
      [
        ...
        { text: `+${progress}`, fillStyle: '#6bff30', font: '12px HelveticaBoldMS' }
      ],
      690,
      y
    )
...
export default new InteractionCommand({
  ...
  async run (interaction: ChatInputCommandInteraction<'cached'>, args: InteractionArgs) {
    ...
    const serverData: CharacterData[] = server === 'global' ? global.weeklyPlayerData : global.weeklyPlayerData.filter((x) => x.server === server)
    ...
    const image = await createWeeklyRankingImage(client, serverData, server, args.language)
