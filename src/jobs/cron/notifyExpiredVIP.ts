...
export async function notifyExpiredVIP(client: Biridim): Promise<void> {
  ...
  for (const user of users) {
    ...
    if (discordUser) {
      ...
      const expirationDate = Math.floor(new Date(user.vip_data.expiration_date as Date).getTime() / 1000);

      const embed = new CustomEmbed()
        .setTitle(client.translate('JOB_NOTIFY_EXPIRED_MESSAGE_TITLE', language))
        .setDescription(
      ...
      discordUser
        .send({ embeds: [embed] })
        .catch(() => {
          logger.error('notifyExpiredVIP', `Could not send message to user ${user.id}`);
        })
        .finally(() => {
          logger.audit('notifyExpiredVIP', `Expired VIP ended for ${user.id} at ${new Date()} with ${expirationDate} as expiration date`);
        });
    }
