...
export function calculatePremiumDays (currentDate: Date, coins: number): { premiumDays: number, premiumExpiration: Date } {
  ...
  const premiumExpiration = new Date(date.getTime() + premiumDays * 24 * 60 * 60 * 1000)
...
export function getLanguageName (languageCode: string): SupportedLanguagesString {
  switch (languageCode) {
    case 'ES':
...
export function getMarketplaceStatsString ({ item }: IOfferItemDocument): string {
  return `${
...
export function getMarketplcaePriceString ({ item }: IOfferItemDocument): string {
  return `${
    item.price.gold > 0
...
export function getMaxExperienceAtLevel (level: number): number {
  ...
  return Math.floor((Math.floor((10 * level * level * level * level) / 200) * level) / 1200) - 1
}
...
export function getEquipmentUpgradeResources (value: number): EquipmentResources {
  let cost = 10000
  const resources = {
    copper: 0,
    tin: 0,
...
export class CustomEmbed {
  ...
  constructor () {
    this.data = new EmbedBuilder()
      .setAuthor({ name: 'Biridim', iconURL: KAKELE_ITEM_ICONS_URL.ElderVampireBrooch })
      .setColor(0xe2725b)
  }
