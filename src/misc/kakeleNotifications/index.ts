  ...
  for (const server in data) {
    ...
    if (serverEvents.length > 0) {
      for (const event of serverEvents) {
        const index = dbEvents.findIndex((x) => x.id === event.id && x.server === server)
  ...
  for (const server in data) {
    ...
    if (serverBoosts.length > 0) {
      for (const boost of serverBoosts) {
        ...
        if (index === -1) {
          const newBoost = await Event.create({
            id: boost.id,
            server,
            timestamp: now,
...
function filterEventGuilds (arr: IGuild[], eventId: string, server: string): EventFiltered[] {
  ...
  for (const guild of arr) {
    ...
    const payers = guild.vip?.payers.filter(
      (x: IUser) => {
        ...
        for (const guild of filteredGuilds) {
          if (guild.payers) {
            ...
            webhookClient
              ...
              .catch((e) => {
                logger.error('notifyEvents', e.message, guild.id, guild.channel)
              })
          }
        }
      }
        ...
        for (const guild of filteredGuilds) {
          if (guild.payers) {
            ...
            webhookClient
              ...
              .catch((e) => {
                logger.error('notifyBoosts', e.message, guild.id, guild.data.channels)
              })
          }
        }
      }
