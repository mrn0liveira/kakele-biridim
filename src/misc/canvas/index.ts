...
export function fillMixedText (ctx: Canvas.CanvasRenderingContext2D, args: Array<{ text: string, fillStyle?: string, font?: string }>, x: number, y: number): void {
...
export async function createItemListImage (items: IOfferItemDocument[], pageString: string = '1/1'): Promise<Canvas.Canvas> {
  ...
  function drawPill (x0: number, y0: number, x1: number, y1: number, r: number, color: string): void {
    const w = x1 - x0
    const h = y1 - y0
  ...
  function drawPayment (
    ...
    if (price.money.currency.length > 0 && price.money.amount > 0) {
      const numberFormat = `${Intl.NumberFormat().format(price.money.amount)} ${price.money.currency.toUpperCase()} `

      ctx.fillStyle = '#42f557'
      ctx.fillText(numberFormat, 120 + ctx.measureText(textArr.join(' ')).width + 10, 198 + index * 52)
  ...
  for (let index = 0; index < 15; index++) {
    ...
    if (item === undefined) {
      ...
      drawPill(120, 162 + index * 52, randomNameShadowWidth, 162 + index * 52 + 20, 7, '#ff8269')
      drawPill(120, 188 + index * 52, randomValueShadowWidth, 188 + index * 52 + 12, 7, '#ff8269')
    ...
    const itemImage = await loadImage(`./src/assets/sprites/items/${item.item.name}.png`)

    ctx.drawImage(itemImage, 90 - (itemImage.width / 2) * 0.07, 180 + index * 52 - (itemImage.height / 2) * 0.07, itemImage.width * 0.07, itemImage.height * 0.07)
    ...
    const amountText = `${new Intl.NumberFormat().format(item.amount > 0 ? item.amount : 1)}x`

    ctx.fillText(amountText, 120, 162 + index * 52)

    if (item.servers.includes('global')) {
      ctx.fillStyle = '#ff9d8a'
      ctx.fillText('Global', 125 + ctx.measureText(amountText).width, 162 + index * 52)
    } else {
      ...
      for (let i = 0; i < item.servers.length; i++) {
        ...
        ctx.beginPath()
        ctx.rect(125 + ctx.measureText(amountText).width + serversSquaresWidth, 155 + index * 52, 7, 7)
    ...
    ctx.fillText(`${item._id.toHexString()} - ${date}`, 125 + itemMameWidth, 182 + index * 52)
